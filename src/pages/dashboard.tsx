...
import { getOfficeManagers } from 'apiSdk/office-managers';
import { OfficeManagerInterface } from 'interfaces/office-manager';
import { getItSupports } from 'apiSdk/it-supports';
import { ItSupportInterface } from 'interfaces/it-support';
import { getAppointments } from 'apiSdk/appointments';
import { AppointmentInterface } from 'interfaces/appointment';

import { OfficeManagerListPage as ShowCaseList } from 'pages/office-managers';

function DashboardPage() {
  const { session } = useSession();

  const officeManagerStat = useSWR<PaginatedInterface<OfficeManagerInterface>>(
    () => `/office-managers`,
    () => getOfficeManagers({ limit: 1, offset: 0 }),
  );

  const itSupportStat = useSWR<PaginatedInterface<ItSupportInterface>>(
    () => `/it-supports`,
    () => getItSupports({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: officeManagerStat?.data?.totalCount,
      isLoading: officeManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'It Supports',
      result: itSupportStat?.data?.totalCount,
      isLoading: itSupportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Appointments',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Office Managers
                </Text>
                <NextLink href={`office-managers`} passHref legacyBehavior>
                  <Button
