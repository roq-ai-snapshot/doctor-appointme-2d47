...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.it_support?.length > 0) {
      const create_it_support = body.it_support;
      body.it_support = {
        create: create_it_support,
      };
    } else {
      delete body.it_support;
    }
    ...
    if (body?.office_manager?.length > 0) {
      const create_office_manager = body.office_manager;
      body.office_manager = {
        create: create_office_manager,
      };
    } else {
      delete body.office_manager;
    }
